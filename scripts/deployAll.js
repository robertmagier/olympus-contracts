...
async function main() {
  ...
  console.log("MockDAO account: " + MockDAO.address);
  ...
  await tx.wait();
  tx = await frax.approve(treasury.address, largeApproval);
  await tx.wait();

  // Approve dai and frax bonds to spend deployer's DAI and Frax
  tx = await dai.approve(daiBond.address, largeApproval);
  await tx.wait();
  tx = await frax.approve(fraxBond.address, largeApproval);
  await tx.wait();

  // Approve staking and staking helper contact to spend deployer's OHM
  tx = await ohm.approve(staking.address, largeApproval);
  await tx.wait();
