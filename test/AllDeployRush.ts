...
describe("Deploy contract for Staking", function () {
  ...
  it("Deploy dummy price feed", async () => {
    const factory = await ethers.getContractFactory("DummyPriceFeed");
    dummyPriceFeed = await factory.deploy("398791993086", 8);
  });
  ...
  it("Deploy new UniswapPair for WETH RUSHV2", async () => {
    // Deploy Uniswap Factory
    UniswapFactory = await ethers.getContractFactory("UniswapFactory");
    uniswapfactory = await UniswapFactory.deploy(deployer.address);
    // Create Pair
  ...
  it("Add liquidity to rushv2weth 30,000 RUSH to 7 WETH", async () => {
    let deadline = (Date.now() / 1000 + 3600).toFixed(0);
  ...
  it("Deploy OlympusBondDepository for RUSHV2-WETH", async () => {
    ...
    bondDepository = await factory.deploy(
      ...
      calculator.address,
      // ETHUSDPRICE
      dummyPriceFeed.address
    );

    const REWARDMANAGER = 8;
    let tx = await treasury.queue(REWARDMANAGER, bondDepository.address);
    await tx.wait();
    tx = await treasury.toggle(REWARDMANAGER, bondDepository.address, ZEROA);
    await tx.wait();
    ...
    let amount = new BigNumber(1).shiftedBy(13).toFixed();
    await rushv2weth.approve(bondDepository.address, amount);
    await bondDepository.deposit(amount, bondPrice, deployer.address);
  });

  return;
  it("Display Addresses", () => {
    console.log("RUSHV2WETH Bond: " + bondDepository.address);
